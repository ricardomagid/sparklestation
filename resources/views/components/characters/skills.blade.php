@if($format=="desktop") 
    <div class="mx-auto panel-base w-[65%] h-[80%] absolute right-5 top-5 bottom-5 p-5 panel-enter right-panel panel-hidden flex flex-col panel-base" data-panel="skills">
...
@endif
    ...
    <div
        ...
        @foreach ($character->grouped_abilities as $category => $abilities)
            ...
            <div class="p-6 h-full {{ $loop->first ? '' : 'hidden' }}" id="panel-{{ Str::slug($category) }}" data-skill-panel={{ Str::slug($category) }}>
                @if ($category != 'Technique')
                    <div class="flex items-center mb-6 bg-black/15 p-3 rounded-lg">
                        <span class="text-white text-sm mr-3 w-20">Level: <span
                                class="ability-slider-number"></span></span>
                        ...
                        <div class="{{ !$loop->first ? 'pt-6 border-t border-white/15' : '' }}">
                            ...
                            <div class="flex items-center mb-4">
                                <div class="bg-gradient-to-br from-red-700 to-red-900 rounded-full p-2 mr-3 shadow-lg">
                                    <img src="{{ $ability->img($i) }}" alt="{{ $ability->name }}"
                                        class="w-10 h-10">
                                </div>
                                <div>
                                ...
                                <div class="grid grid-cols-2 gap-6 mb-4 bg-black/15 rounded-lg p-4">
                                    @if ($ability->toughness_reduction > 0)
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <span
                                                class="text-white font-bold">{{ $ability->toughness_reduction }}</span>
                                        </div>
                                    @endif
                                    @if ($ability->energy > 0)
                                        <div class="flex items-center space-x-2">
                            ...
                            <div class="bg-white/5 rounded-lg p-4">
                                ...
                                <p class="text-white text-[15px] leading-relaxed ability-description" data-base-numbers='@json($ability->base_numbers)'
